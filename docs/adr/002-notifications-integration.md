...
4. **Destination resolution** — default binding is `CommunicationDestinationResolver`. It loads an active `CommunicationDestination` for the notifiable morph + channel (preferring primary/verified rows), then falls back to `routeNotificationFor()` / `routeNotificationFor{Driver}` (the same path as `NotifiableDestinationResolver`). Resolved addresses are encrypted, hashed, and hinted via the destination protector before delivery storage.

**Consequences:** Existing application code is untouched. The two modes coexist — managed takes precedence when `communicationId` is present, auto-capture creates records for any notification without context. Apps can pre-store channel destinations without changing notifiable models. The `HasCommunicationContext` trait must remain serialization-safe (scalar identifiers only, no service objects or models).
